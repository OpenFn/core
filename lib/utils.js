...
function getModuleDetails(languagePack) {
  return [...require.resolve.paths(languagePack), '.', process.cwd()].reduce(
    (result, p) => {
      if (result) return result;

      const maybePath = path.resolve(p, languagePack) + '/package.json';

      if (fs.statSync(maybePath, { throwIfNoEntry: false })) {
        return getPackageJson(maybePath);
      }
    },
    null
  );
